"""Authenticated, bounded Comments admission before scheduler reduction."""
...
class CommentAccessToken:
    """One expiring token authorized to submit Comments envelopes."""
...
class CommentIngressConfig:
    """Fixed admission bounds for one authenticated Comments connection."""
...
class CommentIngressRejection(StrEnum):
    """Closed admission refusals that never reach the scheduler."""
...
class CommentIngressReceipt:
    """One correlated admission or reduction outcome."""
...
class AuthenticatedCommentIngress:
    """Own bounded Comments replay state until typed reduction is requested."""
    ...
    def receive(
        ...
        """Authenticate and queue one current comments envelope without reducing it."""
    ...
    def reduce_next(self) -> CommentIngressReceipt:
        """Reduce the next authenticated proposal through the session authority."""
    ...
    def take_next(self) -> CommentProposal | None:
        """Transfer one authenticated proposal to the scheduler-owned runtime."""
        if not self._pending:
            return None
        return self._pending.popleft()

    def cancel_pending(self) -> None:
        """Discard queued comments when the owning connection is cancelled."""
        self._pending.clear()
    ...
    def _authorization_rejection(
        ...
        if token is None:
            return (
                ...
                else CommentIngressRejection.UNAUTHORIZED
            )
        ...
        if not hmac.compare_digest(
            authorization.removeprefix(prefix), token.value
        ):
            return CommentIngressRejection.UNAUTHORIZED
        return None
