"""Redacted operational facts for safe metrics and release artifacts."""
...
class OperationalRecord:
    """One bounded lifecycle fact before it crosses the observability boundary."""
...
class RedactedOperationalRecord:
    """One joinable lifecycle fact safe for operational artifacts."""
...
class OperationalJournal:
    """Own append-only redacted records for one Orchestrator process."""

    def __init__(self) -> None:
        """Create an empty safe operational artifact."""
        self._records: list[RedactedOperationalRecord] = []

    @property
    def records(self) -> tuple[RedactedOperationalRecord, ...]:
        """Return immutable release-safe lifecycle facts."""
        return tuple(self._records)

    def append(self, record: OperationalRecord) -> None:
        """Redact identifiers before retaining an operational lifecycle fact."""
