"""Create and verify canonical Task 8 release certificates."""
...
EXPECTED_COMMANDS: Final = (
    ...
    ("bitnp-raspberrygirl-vtuber-orchestrator", "uv\u001frun\u001fruff\u001fcheck\u001fsrc\u001ftests"),
    ("bitnp-raspberrygirl-vtuber-orchestrator", "python\u001fscripts/verify_protocol_schema.py"),
    ("bitnp-raspberrygirl-vtuber-orchestrator", "python\u001fscripts/verify_topology.py\u001f--sibling-root\u001f.."),
    ("bitnp-raspberrygirl-vtuber-orchestrator", "python\u001fscripts/verify_vtuber_contract.py\u001f--frontend-path\u001f../bitnp-raspberrygirl-vtuber-frontend"),
    ("bitnp-raspberrygirl-vtuber-orchestrator", "bash\u001fscripts/verify_workspace.sh\u001f--sibling-root\u001f.."),
    ("bitnp-raspberrygirl-vtuber-mic", "uv\u001fsync\u001f--locked"),("bitnp-raspberrygirl-vtuber-mic", "uv\u001frun\u001fpytest"),
    ("bitnp-raspberrygirl-vtuber-sound", "uv\u001fsync\u001f--locked"),("bitnp-raspberrygirl-vtuber-sound", "uv\u001frun\u001fpytest"),
    ("bitnp-raspberrygirl-vtuber-comments", "uv\u001fsync\u001f--locked"),("bitnp-raspberrygirl-vtuber-comments", "uv\u001frun\u001fpytest"),
    ("bitnp-raspberrygirl-vtuber-frontend", "godot\u001f--headless\u001f--path\u001f.\u001f--script\u001fres://tests/protocol_smoke.gd"),
)


@dataclass(frozen=True, slots=True)
class CertificateRequest:
    """Trusted locations and identities bound by a Task 8 certificate."""
...
class CertificateVerification:
    """Machine-readable Task 8 certificate verification outcome."""
...
def write_certificate(request: CertificateRequest) -> None:
    """Write a certificate only when both successful manifests are identical."""
...
def verify_certificate(
    ...
    """Verify canonical identity, manifest contents, and derived digest."""
    try:
...
def _manifest_hashes_match(
    ...
    if not all(isinstance(digest, str) for digest in value):
        return False
    return (
        value[0] == _sha256(request.first_manifest)
        and value[1] == _sha256(request.second_manifest)
    )
...
def _manifest_valid(path: Path) -> bool:
    ...
    for index, expected in enumerate(EXPECTED_COMMANDS):
        ...
        if (record.get("repository"), record.get("arguments"), record.get("returncode")) != (*expected, 0):
            return False
