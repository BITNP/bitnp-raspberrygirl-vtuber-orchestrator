"""The sole scheduler validation path from task results to effects."""
...
class TaskEffect:
    """One typed effect proposal produced by an already-authorized task."""
...
@dataclass(frozen=True, slots=True)
class TaskResult:
    """A worker result carrying the task's creation-time scheduler snapshot."""
...
class TaskResultRejection(StrEnum):
    """Closed reasons a task result cannot commit its proposed effect."""
...
class TaskResultAccepted:
    """A reducer-approved effect that the scheduler may now dispatch."""
...
class TaskResultRejected:
    """A reducer refusal that guarantees no effect was committed."""
...
class TaskResultReducer:
    """Validate worker completions against current task lifecycle state."""

    def __init__(self, registry: TaskRegistry) -> None:
        """Bind the reducer to one scheduler-owned task registry."""
    ...
    def reduce(
        ...
        *,
        ...
        """Accept exactly one current result or reject it without an effect."""
