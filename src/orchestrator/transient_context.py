"""Session-local finalized LLM context with deterministic compaction."""
...
class ContextProvenance:
    """Source identity retained in every context entry and digest."""
...
class FinalizedInput:
    """Finalized audience text eligible for one session's context."""
...
class AcceptedOutput:
    """Reducer-accepted answer text eligible for one session's context."""
...
class PartialMaterial:
    """Incomplete material that must never become context."""
...
class CancelledMaterial:
    """Cancelled material that must never become context."""
...
@dataclass(frozen=True, slots=True)
class StaleMaterial:
    """Superseded material that must never become context."""
...
class ContextEntryKind(StrEnum):
    """Closed roles admitted to transient context."""
...
@dataclass(frozen=True, slots=True)
class ContextEntry:
    """One admitted text item with immutable source provenance."""
...
class TransientContextSnapshot:
    """Immutable session-local state consumable by future task reducers."""
...
class ModelContextBudget:
    """Input-token capacity allocated to transient context for one model."""

    input_tokens: TokenBudget

    def __post_init__(self) -> None:
        """Reject capacities that cannot carry a deterministic digest."""
        if self.input_tokens <= 0:
            raise InvalidContextBudgetError(input_tokens=self.input_tokens)


class ContextBudgetPolicy(Protocol):
    """Select a model-specific context budget without contacting a provider."""

    def budget_for(self, model_id: ModelId) -> ModelContextBudget:
        """Return the local composition budget for one configured model."""
        ...


@dataclass(frozen=True, slots=True)
class StaticContextBudgetPolicy:
    """One deterministic model budget for tests and local deployments."""

    model_id: ModelId
    budget: ModelContextBudget

    def budget_for(self, model_id: ModelId) -> ModelContextBudget:
        """Return the configured budget only for its model identity."""
...
@dataclass(frozen=True, slots=True)
class ContextDigest:
    """Deterministic compacted representation with complete source identities."""
...
class ContextComposition:
    """Bounded context snapshot prepared for a single model invocation."""
...
class ContextSessionMismatchError(Exception):
    """Raised when material from another session is offered to a context."""
...
class InvalidContextBudgetError(Exception):
    """Raised when a model budget cannot contain a provenance digest."""
...
class ModelBudgetUnavailableError(Exception):
    """Raised when no local budget exists for a requested model."""
...
class TransientContext:
    """Mutable accumulator whose lifetime is restricted to one session."""

    def __init__(self, *, session_id: SessionId) -> None:
        """Create empty, non-persistent context for one session identity."""
    ...
    def snapshot(self) -> TransientContextSnapshot:
        """Return the current immutable session-local context state."""
    ...
    def consider(self, material: ContextMaterial) -> TransientContextSnapshot:
        """Admit only finalized input or reducer-accepted output material."""
    ...
    def reset(self) -> TransientContextSnapshot:
        """Clear context and advance the observable session-local generation."""
        self._entries.clear()
        self._generation += 1
        return self.snapshot
    ...
    def compose(
        ...
        """Compose a deterministic bounded view for one configured model."""
...
def compose_context(
    ...
    """Compact oldest entries first while retaining newest raw text."""
