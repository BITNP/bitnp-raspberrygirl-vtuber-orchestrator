"""Validated 16 kHz PCM16LE chunk conversion for generated L16 RTP."""
...
class PcmChunkError(ValueError):
    """Raised when a provider chunk violates the fixed generated-audio contract."""
...
@dataclass(frozen=True, slots=True)
class Pcm16leChunk:
    """One provider PCM16LE byte chunk under the fixed onsite media contract."""
    ...
    def __post_init__(self) -> None:
        """Reject provider media that is not fixed 16 kHz mono PCM16LE."""
...
class StreamingTtsAdapter(Protocol):
    """TTS capability that emits validated fixed-format PCM chunks."""
    ...
    def stream_pcm16le(
        ...
        *,
        ...
        """Return ordered 16 kHz mono PCM16LE chunks for one answer."""
        ...


@dataclass(slots=True)
class TtsPcmRtpPacketizer:
    """Mutable byte/frame accumulator for exactly one stream cancellation epoch."""
    ...
    def __post_init__(self) -> None:
        """Allocate this epoch's immutable generated SSRC."""
        self.ssrc = generated_ssrc(self.stream, self.cancellation_epoch)

    def push(self, chunk: Pcm16leChunk) -> tuple[bytes, ...]:
        """Buffer one validated chunk and emit every complete 20 ms RTP frame."""
        if self._cancelled:
            return ()
    ...
    def finish(self) -> tuple[bytes, ...]:
        """Emit the zero-padded final frame after rejecting an incomplete sample."""
        if self._cancelled:
            return ()
    ...
    def cancel(self) -> None:
        """Prevent every later provider chunk from producing RTP for this epoch."""
...
def generated_ssrc(stream: StreamKey, cancellation_epoch: CancellationEpoch) -> int:
    """Derive a nonzero generated SSRC that is distinct for each stream epoch."""
