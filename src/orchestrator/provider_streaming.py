"""Shared typed provider streaming boundaries."""
...
class ProviderDeadlines:
    """Independent provider connect, read, and total request budgets."""
...
class ProviderRequest:
    """One typed provider HTTP request and its stage identity."""
...
class ProviderResponseError(OSError):
    """Raised for a provider response that cannot produce a typed event."""
...
class ProviderCancellationHandle:
    """Cancels an active provider read by closing its owned connection."""

    def __init__(self) -> None:
        """Create a mutable cancellation handle with no bound provider work."""
    ...
    def cancelled(self) -> bool:
        """Return whether this provider operation is cancelled."""
        with self._lock:
            return self._cancelled

    @property
    def reason(self) -> str | None:
        """Return the first provider cancellation reason."""
        with self._lock:
            return self._reason

    def cancel(self, *, reason: str) -> bool:
        """Cancel once and close every currently bound provider connection."""
        with self._lock:
            if self._cancelled:
                return False
    ...
    def bind(self, callback: Callable[[], None]) -> Callable[[], None]:
        """Bind a resource close action and return its cleanup callback."""
        with self._lock:
            if self._cancelled:
                _ = callback()
                return _noop
...
def post_sse(
    ...
    """POST a provider request and yield complete SSE data fields in order."""
...
def post_bytes(
    ...
    """POST one final-only provider request under independent deadlines."""
    connection = _connection(request.url, deadlines, request.stage)
    release = _noop if cancellation is None else cancellation.bind(connection.close)
    response_release = _noop
    started = time.monotonic()
...
def _noop() -> None:
    """Release no resource when no cancellation handle was supplied."""
