"""Deterministic canonical-RTP VAD endpointing for one routed input stream."""
...
class EndpointReason(StrEnum):
    """The deterministic event that closes an input utterance."""
...
class PartialUtterance:
    """One accepted speech window before its eventual endpoint."""
...
class EndpointedUtterance:
    """A bounded utterance carrying stream-local lifecycle identifiers."""
...
class StreamEndpointer:
    """Accumulate canonical 20 ms frames and emit bounded VAD utterances."""
    ...
    def push(self, packet: bytes) -> tuple[EndpointEvent, ...]:
        """Accept one validated RTP packet, dropping duplicates and closing gaps."""
    ...
    def disconnect(self) -> EndpointedUtterance | None:
        """Close pending speech once when the route owner disconnects."""
    ...
    def _finalize(self, reason: EndpointReason) -> list[EndpointedUtterance]:
        ...
        event = EndpointedUtterance(
            ...
            reason=reason,
            turn_id=TurnId(f"{self.stream.session_id}:{self.stream.stream_id}:turn:{self._turn_index}"),
            segment_id=SegmentId(f"{self.stream.session_id}:{self.stream.stream_id}:segment:{self._segment_index}"),
