...
_RESPONSE_PROTOCOL = _inline_prompt(
    ...
    【输出语法】只输出一个 JSON 对象，不得输出 Markdown、代码围栏、解释文字或第二个对象。其语法必须为：{"schema_version":"2.0.0","decision":"accept"|"discard","speech":string,"operation":null|{"intent":string,"arguments":object}}。顶层字段必须且只能是 schema_version、decision、speech、operation；operation 非 null 时字段必须且只能是 intent、arguments。禁止计划、操作数组、嵌套操作、工具循环和未声明字段。
    【输出语义】decision=discard 表示当前输入不应进入对话，speech 必须严格为 ""，operation 必须为 null。decision=accept 表示当前输入应进入对话，speech 为 "" 表示明确选择静默；非空时必须是自然、可直接面向用户朗读的内容；speech 只用于 TTS 与字幕，绝不能夹带操作参数。operation 为 null 表示不请求非语音操作，可以直接回复或静默接纳；非 null 表示在回复之外提议恰好一个操作，其 intent 必须来自 available_operations，arguments 只能包含并严格符合对应 arguments_schema 的模型侧字段，不得自行补充 session、turn、command、路径、令牌或其他运行时字段。接纳、操作和发声分别决策。根据场景自主决定是否在操作前告知、完成后反馈或全程静默；用户不必额外声明不要说话。工具结果尚未返回时不得声称操作已经成功。
    【动作标记】speech 可在与话语语义自然匹配的位置插入动作标记，用于让虚拟形象在字幕播放到该位置时做动作。唯一允许的完整语法和名称是 <action name="hello"/>（问候、欢迎或道别）、<action name="act_cute"/>（轻松、亲切或可爱的表达）、<action name="emphasis"/>（强调当前要点）。允许的面捕表情标记是 <expression name="nod"/>（点头、认同）、<expression name="shake_head"/>（摇头、否定）、<expression name="wink"/>（俏皮眨眼）；这些标记播放预先录制的面部动作。要经常使用表情动作哦，但也不要太频繁。平均每次回答使用1~2个表情动作。标记不是口语内容，不得改写、嵌套或使用其他 action 或 expression 名称；带语音的动作标记只属于 speech 时间线。无声动作必须使用 available_operations 中允许的 avatar.cue 操作，speech 设为 ""，不得制造空音频或仅有标签的 speech。
    【判定规则】对于 source=asr：若 was_playing_1000ms_ago=true 且本次请求语音，只有 input.text 明确要求停止、打断、纠正或切换话题时才能 accept，其余必须 discard；该规则不适用于 comment（即 source=comment）。ASR text 若过短、残缺、乱码、不知所云，或只有请求用户重复才能处理，必须 discard；绝不能 accept 后复述识别文本、声称没听清或请求用户重复。接受明确打断时仍自主决定是否发声，不要虚构 interrupt 操作。对于 source=comment，按内容相关性正常判定。
    【操作结果阶段】若 stage="操作结果回复" 或存在 tool_observation，这是唯一一次结果回复：必须输出 decision="accept"、基于 tool_observation 中 status 与 text 自主选择真实 speech 或 "" 静默结束，并令 operation=null；失败时不得编造成功；可以如实反馈或保持静默，不得再次提议操作。
    """
)
...
class AsyncCompositeResponseToolExecutor:
    def __init__(
        ...
        avatar: AsyncResponseToolExecutor | None = None,
    ) -> None:
        self._mcp = mcp
    ...
    async def execute(
        ...
        if request.kind == "avatar" and self._avatar is not None:
            return await self._avatar.execute(request, snapshot)
...
def build_async_response_coordinator(  # noqa: PLR0913
    ...
    *,
    ...
    avatar_executor: AsyncResponseToolExecutor | None = None,
    ...
    if avatar_executor is not None:
        specs.append(
            IntentSpec(
                "avatar.cue",
                "avatar",
                    ...
                    "oneOf": [
                        {
                            "properties": {
                                "kind": {"const": "action"},
                                "name": {"enum": ["hello", "act_cute", "emphasis"]},
                            }
                        },
                        {
                            "properties": {
                                "kind": {"const": "expression"},
                                "name": {"enum": ["nod", "shake_head", "wink"]},
                            }
                        },
                    ],
                },
                model_label="执行一个已有的无声形象动作；不创建语音或字幕",
                lane="interactive",
                timeout_ms=5000,
            )
        )
    ...
    return AsyncResponseCoordinator(
        ...
        router,
        AsyncCompositeResponseToolExecutor(mcp, presentation_executor, avatar_executor),
        retrieval,
    )
...
_RESPONSE_SCHEMA: dict[str, object] = {
    "type": "object",
    "additionalProperties": False,
    "required": ["schema_version", "decision", "speech", "operation"],
    "properties": {
