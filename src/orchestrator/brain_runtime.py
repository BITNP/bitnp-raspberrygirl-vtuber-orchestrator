...
_RESPONSE_SYSTEM = _inline_prompt(
    ...
    【动作标记】speech 可在与话语语义自然匹配的位置插入动作标记，用于让虚拟形象在字幕播放到该位置时做动作。唯一允许的完整语法和名称是 <action name="hello"/>（问候、欢迎或道别）、<action name="act_cute"/>（轻松、亲切或可爱的表达）、<action name="emphasis"/>（强调当前要点）。要经常使用表情动作哦，但也不要太频繁。平均每次回答使用1~2个表情动作。标记不是口语内容，不得改写、嵌套或使用其他 action 名称；动作标记只是 speech 时间线提示，不是 operation，不得为它创建操作或参数。
