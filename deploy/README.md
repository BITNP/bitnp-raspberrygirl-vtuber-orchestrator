...
5. Configure Mic and Sound with the same `BITNP_SESSION_ID` or `SOUND_SESSION_ID` and the same `BITNP_MIC_STREAM_ID` or `SOUND_RTP_STREAM_ID`. Configure Mic, Sound, and Comments with `wss://<orchestrator-host>/control`, the role-specific trusted-LAN token, and the shared CA-bundle path. Mic has no RTP host or port. No Mic-to-Sound address belongs in either file.
