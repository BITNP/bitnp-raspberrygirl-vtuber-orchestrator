...
```text
Mic VAD/ASR -> Orchestrator Brain/OpenAI-compatible TTS -> Sound
```
...
4. Set `ORCHESTRATOR_LLM_PROVIDER=openai_compatible`, explicitly select `ORCHESTRATOR_LLM_REASONING_DIALECT=deepseek`, `openai`, or `none`, and choose `ORCHESTRATOR_TTS_PROVIDER=vllm_omni`, `qwen3ttscpp`, `audio_cpp`, or `aliyun_cosyvoice`. Configure `MIC_ASR_ENDPOINT` and `MIC_ASR_MODEL` in Mic's environment. Supply the selected LLM/TTS provider endpoint and model variables in the Orchestrator example, plus the required API keys. Optional Brain and Maintenance model values fall back to `ORCHESTRATOR_LLM_MODEL`; generation parameters may be set globally or per workload as documented in the [user guide](../docs/user.zh-CN.md#llm-生成参数). With `vllm_omni`, supply nonempty `ORCHESTRATOR_TTS_REF_AUDIO` and `ORCHESTRATOR_TTS_REF_TEXT`; the cloned speaker is selected from that reference, so `ORCHESTRATOR_TTS_VOICE` remains blank. `ORCHESTRATOR_TTS_REF_AUDIO` may be an absolute local WAV path, a `file://` WAV URI, or an existing `data:audio/wav;base64,...` URL. Orchestrator encodes local paths and file URIs into the TTS request so the provider does not need host filesystem access; do not put a real data URL with private voice material in source control. With `qwen3ttscpp` (the qwentts.cpp engine), put one speaker id registered in the engine in `ORCHESTRATOR_TTS_VOICE`; it selects the speaker by that id alone, so leave both reference fields blank. `ORCHESTRATOR_TTS_MODE=streaming` then requests the engine's chunked raw s16le 24 kHz mono PCM, which Orchestrator resamples to 16 kHz and paces onto RTP, while `final_only` requests one complete WAV clip. With `audio_cpp`, use the provider's speaker identifier in `ORCHESTRATOR_TTS_VOICE`; reference audio and text are not required. With `aliyun_cosyvoice`, set `ORCHESTRATOR_TTS_ENDPOINT` to the Model Studio realtime WebSocket inference endpoint `wss://<workspace-host>/api-ws/v1/inference` (or `wss://dashscope.aliyuncs.com/api-ws/v1/inference` for the shared endpoint), set the API key, and put a system voice or a voice ID created by Alibaba Cloud's separate voice-cloning API in `ORCHESTRATOR_TTS_VOICE`. The model version must match that voice: for example, `longanhuan_v3` requires `cosyvoice-v3-flash`, not `cosyvoice-v3.5-flash`. Leave both reference fields blank because realtime synthesis only accepts the resulting voice ID. Use `ORCHESTRATOR_TTS_MODE=streaming` for incremental 16 kHz PCM events; the legacy value `streaming_sse` is still accepted. Orchestrator installs `ORCHESTRATOR_TTS_API_KEY` as the DashScope SDK key and does not apply `ORCHESTRATOR_TLS_CA_PATH` to this provider, because that SDK owns its WebSocket TLS configuration.
5. Configure Mic and Sound with the same `BITNP_SESSION_ID` or `SOUND_SESSION_ID` and the same `BITNP_MIC_STREAM_ID` or `SOUND_RTP_STREAM_ID`. The default secure deployment configures Mic, Sound, and Comments with `wss://<orchestrator-host>/control`, the role-specific trusted-LAN token, and the shared CA-bundle path. A physically controlled LAN may instead follow the explicit authenticated `ws://` configuration below. Mic has no RTP host or port. No Mic-to-Sound address belongs in either file.
...
This rejects missing PEM CA-bundle paths, non-Orchestrator control endpoints, Mic/Sound direct-peer endpoints, and unequal `BITNP_SESSION_ID` / `SOUND_SESSION_ID` or `BITNP_MIC_STREAM_ID` / `SOUND_RTP_STREAM_ID` values.

## Single-Host Model Servers

Mic ASR and Orchestrator TTS model servers on the same host may use loopback HTTP endpoints such as `http://127.0.0.1:8090/v1/audio/transcriptions` and `http://127.0.0.1:8091/v1`. This exception applies only to the private, same-host provider boundary. WSS remains the default control-plane deployment. A physically controlled LAN may instead use the explicit authenticated plaintext mode documented in the [trusted-LAN guide](../docs/local-loopback.zh-CN.md); distinct role tokens remain mandatory.
...
6. For latency or audio diagnostics, inspect Mic's endpoint/ASR logs, Orchestrator's candidate/Brain/TTS records, and Sound's `rtp_ingress_diagnostic`. A healthy generated stream has `drops=0` and `late_gaps=0`; use these records to distinguish endpointing, ASR, Brain admission, TTS, RTP, and playback delay. `BITNP_LOG_LEVEL=DEBUG` also records the effective LLM generation parameters, without the API key.
7. For a normal shutdown or a failed test, stop Mic, then Sound, then Orchestrator. Restore the prior protected environment files and start in the documented order. Do not change schemas or open a direct Mic-to-Sound path.

Authenticated plaintext operation on a physically controlled LAN is documented separately in the [trusted-LAN guide](../docs/local-loopback.zh-CN.md). It must never be exposed to an untrusted network or the public Internet.
