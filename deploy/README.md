...
4. Set `ORCHESTRATOR_LLM_PROVIDER=openai_compatible`, explicitly select `ORCHESTRATOR_LLM_REASONING_DIALECT=deepseek`, `openai`, or `none`, and choose `ORCHESTRATOR_TTS_PROVIDER=vllm_omni` or `audio_cpp`. Configure `MIC_ASR_ENDPOINT` and `MIC_ASR_MODEL` in Mic's environment. Supply the selected LLM/TTS provider endpoint and model variables in the Orchestrator example, plus the LLM API key when required. Optional Brain and Maintenance model values fall back to `ORCHESTRATOR_LLM_MODEL`; generation parameters may be set globally or per workload as documented in the [user guide](../docs/user.zh-CN.md#llm-生成参数). With `vllm_omni`, supply nonempty `ORCHESTRATOR_TTS_REF_AUDIO` and `ORCHESTRATOR_TTS_REF_TEXT`; the cloned speaker is selected from that reference, so `ORCHESTRATOR_TTS_VOICE` remains blank. `ORCHESTRATOR_TTS_REF_AUDIO` may be an absolute local WAV path, a `file://` WAV URI, or an existing `data:audio/wav;base64,...` URL. Orchestrator encodes local paths and file URIs into the TTS request so the provider does not need host filesystem access; do not put a real data URL with private voice material in source control. With `audio_cpp`, use the provider's speaker identifier in `ORCHESTRATOR_TTS_VOICE`; reference audio and text are not required.
