...
Raspberry Girl 是一个面向公开讲解、虚拟主播和现场产品介绍的多模态 AI 角色系统。它把听、说、读、推理、记忆、预设动作、演示控制和观众评论接入到一个由 Orchestrator 统一调度的体验中，目标是在演讲、展台和直播场景里提供响应及时、边界清晰的 AI 讲解员。
...
- 模式：`lecturer`、`virtual_streamer` 和 `onsite_explainer`。只有 Orchestrator 和 Frontend 感知模式，其他模块保持模式无关。
...
现场讲解使用 Orchestrator 拥有的音频替换链路：Mic 把 L16 RTP 发给 Orchestrator，Orchestrator 完成 VAD、ASR、LLM 和 vLLM-Omni TTS，再把生成的 L16 RTP 发给 Sound。Frontend 不参与这个音频部署。
...
`virtual_streamer` 和 `lecturer` 通过 Orchestrator 向 Frontend 发送有限的动作、表情、场景和演示命令。LLM 输出只是候选提案，真正执行前会经过 typed command、能力 allowlist、当前状态和前置条件校验。
