...
Raspberry Girl is one adaptive multimodal AI agent rather than a set of fixed product modes. Orchestrator chooses the current interaction strategy from session state, input sources, active capabilities, and user intent. The frontend executes only explicit Orchestrator commands, while Mic, Comments, and Sound remain strategy-agnostic transport clients. Orchestrator owns configurable OpenAI-compatible ASR, LLM, and TTS providers, including vLLM-Omni voice cloning. NATS, JetStream, WebRTC, full RAG, and production Bilibili are deferred.
