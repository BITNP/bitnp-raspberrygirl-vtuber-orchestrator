#!/usr/bin/env python3
"""Verify canonical protocol fixtures through JSON Schema and semantic rules."""
...
def parse_args() -> argparse.Namespace:
    """Parse the optional invalid-fixture assertion input."""
    parser = argparse.ArgumentParser(description="Validate canonical protocol schemas and fixtures.")
    _ = parser.add_argument("--expect-invalid", type=Path)
    return parser.parse_args()


def _report(errors: list[ProtocolValidationError]) -> None:
    print(json.dumps({"accepted": False, "errors": [error.as_json() for error in errors]}, sort_keys=True))


def main() -> int:
    """Run the protocol fixture verification command."""
    args = parse_args()
    ...
    if valid_errors or invalid_failures:
        ...
        if invalid_failures:
            print(json.dumps({"accepted": True, "invalid_fixtures": [str(path) for path in invalid_failures]}, sort_keys=True))
        return 1
