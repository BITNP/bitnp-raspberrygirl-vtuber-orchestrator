...
def _environment_errors(environments: dict[str, dict[str, str]]) -> list[str]:
    ...
    control_port = orchestrator.get("ORCHESTRATOR_TRANSPORT_ADVERTISED_CONTROL_PORT", "")
    ...
    if mic.get("ORCHESTRATOR_WS_URL") != expected_url:
        found.append("mic: control endpoint must be the advertised Orchestrator WSS URL")
    if sound.get("ORCHESTRATOR_WS_URL") != expected_url:
        found.append("sound: control endpoint must be the advertised Orchestrator WSS URL")
    if mic.get("ORCHESTRATOR_RTP_HOST") != host or mic.get("ORCHESTRATOR_RTP_PORT") != rtp_port:
        found.append("mic: RTP endpoint must be the advertised Orchestrator RTP endpoint")
