...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify the frontend contract without Godot.")
...
def apply(state: State, event: JsonObject) -> tuple[State, bool]:
    ...
    if event.get("source") != "orchestrator" or data is None or not isinstance(event_type, str):
        return state, False
    match event_type:
...
def main() -> int:
    ...
    if "run/orchestrator_ws_url" not in project or "application/run/orchestrator_ws_url" not in client:
        print("Orchestrator WebSocket setting is missing")
        return 1
    if any(setting in project for setting in ("asr_ws_url", "tts_ws_url", "comments_ws_url", "sound_ws_url", "mic_ws_url")):
