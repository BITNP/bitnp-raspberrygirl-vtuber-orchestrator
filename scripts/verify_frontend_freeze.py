#!/usr/bin/env python3
"""Reject Frontend worktree drift until the Task 8 certificate authorizes it."""
...
DEFAULT_CERTIFICATE: Final = WORKSPACE / ".omo/evidence/task-8-core-loop-before-frontend.json"
...
class FreezeResult:
    """Machine-consumed Frontend freeze verification result."""
...
@dataclass(frozen=True, slots=True)
class FreezeRequest:
    """Inputs needed to decide pre- and post-Task 8 Frontend admission."""
...
def parse_args() -> argparse.Namespace:
    """Parse explicit repository baseline and certificate paths."""
    parser = argparse.ArgumentParser(description="Verify the pre-Task-8 Frontend freeze.")
...
def _changed_paths(frontend: Path, baseline: str) -> tuple[str, ...] | None:
    ...
    untracked = _git(frontend, "ls-files", "--others", "--exclude-standard").stdout.splitlines()
    return tuple(sorted(set(tracked) | set(untracked)))


def verify(request: FreezeRequest) -> FreezeResult:
    """Evaluate the Frontend release authorization against its approved baseline."""
...
def main() -> int:
    """Run the Frontend freeze CLI and print one structured result."""
    args = parse_args()
    result = verify(
        FreezeRequest(
    ...
    print(json.dumps({"accepted": result.accepted, "code": result.code, "paths": result.paths}, sort_keys=True))
